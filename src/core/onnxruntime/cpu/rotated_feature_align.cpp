...
void rotated_feature_align_forward_cpu_kernel(const int nthreads,
                                              const int points,
                                              const scalar_t* bottom_data,
                                              const scalar_t* best_bboxes,
                                              ...
                                              scalar_t* top_data)
{
    for (int index = 0; index < nthreads; index++) {
        ...
        const scalar_t* bbox_offset = best_bboxes + ((n * height + h) * width + w) * 5;
        ...
        const scalar_t* offset_bottom_data = bottom_data + (n * channels + c) * height * width;

        scalar_t output_val = bottom_data[index];
        for (int i = 0; i < points; i++) {
            output_val += bilinear_interpolate<scalar_t>(offset_bottom_data, height, width, py[i], px[i], i);
        }
...
void MMCVRotatedFeatureAlignKernel::Compute(OrtKernelContext* context)
{
    ...
    const float* features_data = reinterpret_cast<const float*>(ort_.GetTensorData<float>(input_features));
    const OrtValue* input_best_rbboxes = ort_.KernelContext_GetInput(context, 1);
    const float* best_rbboxes = reinterpret_cast<const float*>(ort_.GetTensorData<const float*>(input_best_rbboxes));
    ...
    for (size_t i = 1; i < out_dimensions.size(); ++i) {
        output_size *= out_dimensions.data()[i];
    }
    rotated_feature_align_forward_cpu_kernel<float>(output_size,
                                                    points_,
                                                    features_data,
                                                    best_rbboxes,
                                                    spatial_scale_,
