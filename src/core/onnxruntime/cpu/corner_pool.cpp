...
void TopPoolForwardCPU(
    const float* input, float* output, const int batch_size, const int channels, const int height, const int width)
{
            ...
            for (int w = 0; w < width; w++) {
                ...
                for (int h = height - 2; h >= 0; h--) {
                    output[index_n_c + h * width + w] =
                        std::max(output[index_n_c + (h + 1) * width + w], input[index_n_c + h * width + w]);
                }  // for h
            }      // for w
        }          // for c
    }              // for n
        ...
        for (int c = 0; c < channels; c++) {
            ...
            for (int h = 0; h < height; h++) {
                ...
                for (int w = width - 2; w >= 0; w--) {
                    output[index_n_c + h * width + w] =
                        std::max(output[index_n_c + h * width + w + 1], input[index_n_c + h * width + w]);
                }  // for w
            }      // for h
        }          // for c
    }              // for n
...
void MMCVCornerPoolKernel::Compute(OrtKernelContext* context)
{
    ...
    if (mode == 0)
        TopPoolForwardCPU(input_data, output_data, batch_size, input_channels, input_height, input_width);
    else if (mode == 1)
        BottomPoolForwardCPU(input_data, output_data, batch_size, input_channels, input_height, input_width);
    else if (mode == 2)
        LeftPoolForwardCPU(input_data, output_data, batch_size, input_channels, input_height, input_width);
    else
        RightPoolForwardCPU(input_data, output_data, batch_size, input_channels, input_height, input_width);
}
