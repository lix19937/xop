...
float bilinear_interpolate_2d(const float* src, const int64_t src_h, const int64_t src_w, const float h, const float w)
{
...
void deformable_im2col_2d(const float* input,
                          const float* offset,
                          const float* mask,
                          const int64_t src_h,
                          const int64_t src_w,
...
void gemm_ref_fp32(const float* A,
                   const float* B,
                   const float* V,
                   const float* H,
...
void deformable_conv2d_ref_fp32(const float* src,
                                const float* offset,
                                const float* mask,
                                const float* filter,
                                const float* bias,
...
void MMCVModulatedDeformConvKernel::Compute(OrtKernelContext* context)
{
    ...
    const float* bias_data =
        (bias != nullptr) ? reinterpret_cast<const float*>(ort_.GetTensorData<float>(bias)) : nullptr;
    ...
    deformable_conv2d_ref_fp32(input_data,
                               offset_data,
                               mask_data,
                               filter_data,
                               bias_data,
