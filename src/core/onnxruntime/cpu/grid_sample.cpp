...
// ref https://github.com/pytorch/pytorch/blob/master/aten/src/ATen/native/GridSampler.cpp
...
grid_sampler_compute_source_index(scalar_t coord, int64_t size, int64_t padding_mode, bool align_corners)
{
...
template<typename scalar_t>
static inline scalar_t get_value_bounded(const scalar_t* data,
                                         scalar_t x,
                                         scalar_t y,
                                         int64_t W,
                                         int64_t H,
                ...
                if (interpolation_mode == GridSamplerInterpolation::Bilinear) {
                    ...
                    for (int64_t c = 0; c < C; ++c, out_ptr_NCHW += out_sC, inp_ptr_NC += inp_sC) {
                        ...
                        for (int64_t i = 0; i < 4; ++i) {
                            coefficients[i] = cubic_interp1d<float>(get_value_bounded<float>(inp_ptr_NC,
                                                                                             ix_nw - 1,
                                                                                             iy_nw - 1 + i,
                                                                                             inp_W,
                                                                                             inp_H,
                                                                                             ...
                                                                                             padding_mode,
                                                                                             align_corners),
                                                                    get_value_bounded<float>(inp_ptr_NC,
                                                                                             ix_nw + 0,
                                                                                             iy_nw - 1 + i,
                                                                                             inp_W,
                                                                                             inp_H,
                        ...
                        *out_ptr_NCHW = cubic_interp1d<float>(
