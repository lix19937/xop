...
float bilinear_interpolate(const float* src, const int64_t src_h, const int64_t src_w, const float h, const float w)
{
                       ...
                       float* columns)
{
    ...
    for (int64_t index = 0; index != indices; ++index) {
        ...
        auto columns_ptr = columns + (c_col * (dst_h * dst_w) + h_col * dst_w + w_col);
        auto input_ptr = input + c_im * (src_h * src_w);
        auto offset_ptr = offset + grp_idx * 2 * kernel_h * kernel_w * dst_h * dst_w;

        for (int64_t kh = 0; kh < kernel_h; ++kh) {
            for (int64_t kw = 0; kw < kernel_w; ++kw) {
                const int data_offset_h_ptr = ((2 * (kh * kernel_w + kw)) * dst_h + h_col) * dst_w + w_col;
                const int data_offset_w_ptr = ((2 * (kh * kernel_w + kw) + 1) * dst_h + h_col) * dst_w + w_col;

                const float offset_h = offset_ptr[data_offset_h_ptr];
                const float offset_w = offset_ptr[data_offset_w_ptr];
                const float ih = (h_col * stride_h - pad_h) + kh * dilation_h + offset_h;
                const float iw = (w_col * stride_w - pad_w) + kw * dilation_w + offset_w;
                             ...
                             float* dst)
{
    ...
    for (int64_t b = 0; b < batch; ++b) {
        for (int64_t g = 0; g < group; ++g) {
            deformable_im2col(src + b * src_c * src_h * src_w + g * ic_per_gp * src_h * src_w,
            ...
            gemm_ref_fp32_deform(filter + g * oc_per_gp * ic_per_gp * kernel_h * kernel_w,
                                 columns,
                                 nullptr,
                                 dst_ptr,
                                 0,
...
void MMCVDeformConvKernel::Compute(OrtKernelContext* context)
{
    ...
    // get output memory
    int64_t out_height =
        floor((in_height + 2 * padding_height - dilation_height * (kernel_height - 1) - 1) / stride_height + 1);
    int64_t out_width =
        floor((in_width + 2 * padding_width - dilation_width * (kernel_width - 1) - 1) / stride_width + 1);

    std::vector<int64_t> output_dims = {batch_size, out_channels, out_height, out_width};
    ...
    // allocate tmp memory
    int64_t column_len = (in_channels / group) * kernel_height * kernel_width * out_height * out_width;
    float* columns = (float*)allocator_.Alloc(sizeof(float) * column_len);
    deformable_conv_forward(input_data,
                            offset_data,
                            filter_data,
                            batch_size,
                            in_channels,
